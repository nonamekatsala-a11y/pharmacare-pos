  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto mb-4 border border-primary-100 rounded-lg bg-gray-50">
        {items.length === 0 ? (
          ...
          <div className="divide-y divide-primary-100">
            {items.map((item) => (
              <div key={item.medicineId} className="p-3 hover:bg-primary-50 transition-colors">
                {/* Item Name */}
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      className="w-full mt-0.5 px-1.5 py-1 border border-primary-200 rounded bg-white text-primary-700 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500"
                    />
                  </div>
                  <div>
                    <p className="text-primary-500 font-semibold text-xs">Price</p>
                    <p className="mt-0.5 text-xs text-primary-700">{formatCurrency(item.unitPrice)}</p>
                  </div>
                  <div>
                    <p className="text-primary-500 font-semibold text-xs">Total</p>
                    <p className="mt-0.5 font-bold text-primary-700 text-xs">{formatCurrency(item.total)}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="space-y-2 border-t border-primary-200 pt-3 mb-3">
        ...
        <div className="flex justify-between items-center border-t border-primary-200 pt-2 text-sm">
      ...
      <div className="space-y-2 border-t border-primary-200 pt-3 mb-3">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-primary-200 rounded-lg bg-primary-50 text-primary-900 text-right text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white"
          />
        </div>
        <div>
          <p className="text-xs text-primary-600 font-semibold mb-0.5">Change</p>
          <p className="text-lg font-bold text-primary-700">{formatCurrency(change)}</p>
        </div>
      </div>
